...
class DeleteAction(Button):
	...
	def deletedSuccess(self, req=None, code=None):
		conf["mainWindow"].log("success", translate("Eintrag gelöscht"), modul=self.parent().parent().module,
							   action="delete")
...
class ListPreviewInlineAction(Button):
	def __init__(self, *args, **kwargs):
		super(ListPreviewInlineAction, self).__init__(translate("vi.sidebar.internalpreview"),
													  icon="icons-list-item", *args, **kwargs)
	...
	def toggleIntPrev(self):
		...
		if len(selection) == 1 and intPrevActive == True:
			preview = InternalPreview(self.parent().parent().module, self.parent().parent()._structure,
									  selection[0])
			self.parent().parent().sideBar.sidebarHeadline.element.innerHTML = translate(
	...
	def isSuitableFor(module, handler, actionName):
		...
		hasAccess = conf["currentUser"] and (
				"root" in conf["currentUser"]["access"] or module + "-view" in conf["currentUser"]["access"])
		generallyDisabled = not bool(
			conf["modules"][module].get("disableInternalPreview", not conf["internalPreview"]))
...
class ActivateSelectionAction(Button):
	def __init__(self, *args, **kwargs):
		super(ActivateSelectionAction, self).__init__(translate("Select"), icon="icons-select-add", *args,
													  **kwargs)
...
class SelectFieldsAction(Button):
	def __init__(self, *args, **kwargs):
		super(SelectFieldsAction, self).__init__(translate("Select fields"), icon="icons-list", *args,
												 **kwargs)
...
class TableItems(html5.Div):
	...
	def onTableChanged(self, table, rowCount):
		...
		if table._dataProvider:
			# self.elementSpan = html5.Span(html5.TextNode(translate("current Page {cpg}, loaded elements: {amt},
			# pages: {pg}",amt=rowCount, pg=pages, cpg=currentpage )))
			self.elementSpan = html5.Span(html5.TextNode(
				translate("loaded elements: {amt}, pages: {pg}", amt=len(table._model), pg=pages)))
		else:
			# self.elementSpan = html5.Span(html5.TextNode(translate("current Page {cpg}, all elements loaded: {amt},
			# pages: {pg}",amt=rowCount, pg=pages, cpg=currentpage)))
			self.elementSpan = html5.Span(html5.TextNode(
...
class ListSelectFilterAction(Button):
	def __init__(self, *args, **kwargs):
		super(ListSelectFilterAction, self).__init__(translate("Select Filter"), icon="icons-search", *args,
													 **kwargs)
	...
	def onAttach(self):
		...
		if module in conf["modules"].keys():
			...
			if "disabledFunctions" in modulConfig.keys() and modulConfig["disabledFunctions"] and "fulltext-search" \
					in \
...
class CreateRecurrentAction(Button):
	def __init__(self, *args, **kwargs):
		super(CreateRecurrentAction, self).__init__(translate("Save-Close"), icon="icons-save-file", *args,
													**kwargs)
...
class ExportCsvAction(Button):
	def __init__(self, *args, **kwargs):
		super(ExportCsvAction, self).__init__(translate("CSV Export"), icon="icons-download-file", *args,
											  **kwargs)
...
class SelectAllAction(Button):
	def __init__(self, *args, **kwargs):
		super(SelectAllAction, self).__init__(translate("Select all"), icon="icons-select-add", *args,
											  **kwargs)
...
class UnSelectAllAction(Button):
	def __init__(self, *args, **kwargs):
		super(UnSelectAllAction, self).__init__(translate("Unselect all"), icon="icons-select-remove", *args,
												**kwargs)
...
class SelectInvertAction(Button):
	def __init__(self, *args, **kwargs):
		super(SelectInvertAction, self).__init__(translate("Invert selection"), icon="icons-select-invert",
												 *args, **kwargs)
